 ...
 */
export async function uploadToAzure(filePath) {
    ...
    if (!containerUrl && !connectionString) {
        throw new Error('AZURE_BLOB_SAS_URL or STORAGE_PRIMARY_CONNECTION_STRING must be defined');
    }
    ...
    if (containerUrl) {
        ...
        try {
            ...
            throw new Error('AZURE_BLOB_SAS_URL must be a valid container SAS URL');
        }
    ...
    if (!nameMatch || !keyMatch) {
        throw new Error('STORAGE_PRIMARY_CONNECTION_STRING must include AccountName and AccountKey');
    }
    const data = readFileSync(filePath);
    return uploadWithAccessKey(nameMatch[1], keyMatch[1], containerName, blobName, data);
}
/**
